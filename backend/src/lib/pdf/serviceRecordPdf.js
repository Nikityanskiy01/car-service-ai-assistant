...
/**
 * @param {{ record: object, vehicle: object, brandName?: string }} opts
 * @returns {Promise<Buffer>}
 */
export function buildServiceRecordPdfBuffer({ record, vehicle, brandName }) {
...
/**
 * @param {{ records: object[], vehicle: object, brandName?: string }} opts
 * @returns {Promise<Buffer>}
 */
export function buildServiceHistoryPdfBuffer({ records, vehicle, brandName }) {
  const fontPath = resolvePdfBodyFontPath();
  if (!fontPath) {
    throw new AppError(
      503,
 ...
 * @param {{ record: object, vehicle: object }} opts
 * @returns {Buffer}
 */
export function buildServiceRecordJpegBuffer({ record, vehicle }) {
  const width = 720;
  const height = 400;
