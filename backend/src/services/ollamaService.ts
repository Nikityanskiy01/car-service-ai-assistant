 ...
 * }} params
 */
function buildProviderRequest(provider, { env, targetModel, messages, temperature, format, options, keepAlive }: any) {
  /** @type */
...
async function requestProviderCompletion(
  ...
  const usage = {
    promptTokens: Number(data?.usage?.prompt_tokens || data?.prompt_eval_count || 0) || 0,
    completionTokens: Number(data?.usage?.completion_tokens || data?.eval_count || 0) || 0,
  };
  return {
