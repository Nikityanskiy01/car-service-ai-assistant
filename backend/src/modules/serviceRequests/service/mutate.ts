...
export async function patchRequestStatus(requestId, user, { status, expectedVersion }: any) {
...
export async function bulkPatchStatuses(user, { ids, status }: any) {
  if (user.role !== 'MANAGER' && user.role !== 'ADMINISTRATOR') {
    throw new AppError(403, 'Недостаточно прав для выполнения действия.', 'FORBIDDEN');
  }
  const uniqueIds = Array.from(new Set((ids || []).map((v) => String(v)))) as string[];
...
export async function bulkAssignToManager(user, { ids, managerId }: any) {
  if (user.role !== 'MANAGER' && user.role !== 'ADMINISTRATOR') {
    throw new AppError(403, 'Недостаточно прав для выполнения действия.', 'FORBIDDEN');
  }
  const uniqueIds = Array.from(new Set((ids || []).map((v) => String(v)))) as string[];
