...
/** Two tabs often refresh the same cookie at once; that is not token theft. */
...
export async function register({ email, password, fullName, phone }, meta: any = {}) {
...
export async function verifyEmail({ email, code }: any) {
  const normalizedEmail = normalizeAuthEmail(email);
...
export async function login({ identifier, email, password }, meta: any = {}) {
...
export async function completeVerifiedLogin(user, method, meta: any = {}) {
  await assertNotLocked(user);
...
export async function loginWithTotp({ challengeToken, code }, meta: any = {}) {
  const user = await securityService.verifyTotpChallenge(challengeToken, code, meta);
  return issueTokens(user, meta);
}

export async function refreshAccessToken(refreshTokenValue, meta: any = {}) {
...
export async function changePassword(userId, { currentPassword, newPassword }: any) {
...
async function issueTokens(user, meta: any = {}) {
  const env = getEnv();
...
export async function issueSession(user, meta: any = {}) {
  return issueTokens(user, meta);
}
