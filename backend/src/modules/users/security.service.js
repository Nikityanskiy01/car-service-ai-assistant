...
export async function revokeSession(userId, sessionId, currentRefreshTokenValue = null, { code } = {}) {
  const currentHash = currentRefreshTokenValue ? hashRefreshToken(currentRefreshTokenValue) : null;
...
export async function revokeOtherSessions(userId, currentRefreshTokenValue, { code } = {}) {
...
export async function assertSensitiveAction(user, { password, code = '' }) {
  const bcrypt = (await import('bcryptjs')).default;
