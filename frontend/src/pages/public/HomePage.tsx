...
const steps = [
  ...
  { title: 'Заявка в сервис', text: 'Одним шагом передайте отчёт мастеру и запишитесь в сервис.' },
];
...
const faqs = [
  {
    ...
    a: 'Нет. Чат даёт предварительный ориентир и чек-лист. Точный диагноз — после осмотра в сервисе.',
  },
  {
          ...
          {works.map((item) => (
            <article key={item.id} className="fm-work-card">
              ...
              <div className="fm-work-body">
                <p>{item.title}</p>
                <span>
                  {[item.problem, item.term].filter(Boolean).join(' · ') || 'Кейс из сервиса'}
                </span>
              </div>
            </article>
          ))}
